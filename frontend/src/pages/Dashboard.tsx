...
const ProviderBadge = ({ provider }: { provider: Provider }) => {
  const cfg: Record<Provider, { label: string; color: string; icon: React.ReactNode }> = {
    github: { label: "GitHub", color: "text-white", icon: <Globe size={11} /> },
    gitlab: { label: "GitLab", color: "text-orange-400", icon: <ExternalLink size={11} /> },
    "self-hosted": { label: "Self-hosted", color: "text-slate-400", icon: <ExternalLink size={11} /> },
  };
...
const StatusBadge = ({ status }: { status: SyncStatus }) => {
  const cfg: Record<SyncStatus, { label: string; color: string; icon: React.ReactNode }> = {
    ...
    syncing: { label: "Syncing", color: "text-blue-400 bg-blue-500/10 border-blue-500/20 animate-pulse", icon: <Loader size={10} className="animate-spin" /> },
...
export default function Dashboard({ navigate }: { navigate: (p: Page) => void }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#0f1115]">
      <Sidebar activePage="dashboard" navigate={navigate} />
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="p-8 max-w-5xl w-full">
          ...
          <section className="mb-10">
            <div className="bg-gradient-to-b from-slate-900 to-[#0f1115] border border-slate-800 rounded-2xl p-7 shadow-xl">
              <h3 className="text-base font-bold text-white mb-1 flex items-center gap-2">
                <Plus size={18} className="text-green-500" /> Add New Repository
              </h3>
              <p className="text-slate-500 text-xs mb-5">
                Paste a GitHub, GitLab or SSH URL to start indexing.
              </p>
          ...
          <section>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500">
                Active Repositories ({filtered.length})
              </h3>
            </div>

            <div className="space-y-3">
              {filtered.length === 0 && (
                <p className="text-slate-600 text-sm py-8 text-center">No projects match your search.</p>
              )}
              {filtered.map(project => (
                <div
                  key={project.id}
                  className="bg-slate-900/40 border border-slate-800 rounded-2xl px-5 py-4 hover:border-slate-700 transition group"
                >
                  <div className="flex items-center gap-5">
                    {/* Name & meta */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <p className="text-sm font-bold text-white group-hover:text-green-400 transition font-mono">
                          {project.name}
                        </p>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      >
                        <RefreshCw
                          size={14}
                          className={project.status === "syncing" ? "animate-spin" : ""}
                        />
                      </ActionBtn>
                      <ActionBtn
                        title="Settings"
