...
import { User, Mail, ShieldCheck, Globe, Clock, ChevronRight } from "lucide-react";
import type { Page } from "../App";

export default function UserInformation({ navigate }: { navigate: (p: Page) => void }) {
  return (
    <div className="flex min-h-screen bg-[#0f1115]">
      <Sidebar activePage="user" navigate={navigate} />
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        ...
        <div className="p-8 max-w-4xl w-full">
          ...
          <div className="bg-slate-900/30 border border-slate-800 rounded-2xl overflow-hidden mb-8">
            <div className="h-28 bg-gradient-to-r from-green-900/20 via-slate-800 to-blue-900/10 border-b border-slate-800" />
            <div className="px-8 pb-8 flex flex-col md:flex-row gap-6">
              <div className="relative -mt-12">
                <div className="w-28 h-28 rounded-2xl bg-[#161b22] border-4 border-[#0f1115] flex items-center justify-center shadow-2xl">
                  <User size={56} className="text-slate-700" />
                </div>
              </div>
              <div className="pt-4 flex-1">
                <h3 className="text-xl font-bold text-white">User2137</h3>
                <p className="text-slate-500 text-sm">Fullstack Developer & Documentation Lead</p>
                <div className="flex gap-3 mt-3">
                  <span className="flex items-center gap-1.5 text-xs text-slate-400 bg-slate-800/50 px-3 py-1 rounded-full border border-slate-700">
                    <Globe size={11} /> Poland, EU
                  </span>
                  <span className="flex items-center gap-1.5 text-xs text-slate-400 bg-slate-800/50 px-3 py-1 rounded-full border border-slate-700">
                    <Clock size={11} /> Joined March 2024
                  </span>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <h4 className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">
                Account Details
              </h4>
